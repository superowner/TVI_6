...
void main (void)
{
	...
	const float maxSteps  = 15.0;
	const float	bumpScale = 0.08;
	const float	specExp   = 80.0;
	const float	eps       = 0.05;

	float   numSteps  = maxSteps + et.z * (minSteps - maxSteps);
	float	step   = 1.0 / numSteps;
	vec2	dtex   = -step * bumpScale * et.xy / et.z;	// adjustment for one layer
	...
	if ( diff > eps )								// no sense checking other case
	{
		...
		ltex          = bumpScale * step * lt.xy / lt.z;
